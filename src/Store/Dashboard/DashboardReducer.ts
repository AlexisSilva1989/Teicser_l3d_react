...
import { 
	LOAD_USER_MENU, SET_LOADING, DISMISS_ERROR, LOGOUT, PUSH_ERRORS, RESET_DASHBOARD, 
	LOGIN, CHANGE_LOCALE, SET_PERMISSIONS, DISMISS_ALL, SET_NOTIFICATION_MODULE, SET_BADGES
...
export function dashboardReducer(state: IDashboardState = initialState, action: DashboardActionType): IDashboardState {
	switch (action.type) {
		...
		case SET_NOTIFICATION_MODULE:
			return $u(state, { notificationModule: { 
