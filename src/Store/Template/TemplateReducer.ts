...
export function templateReducer(state: ITemplateState = initialState, action: TemplateActionType): ITemplateState {
	let open: string[];
	let trigger: string[];
	switch (action.type) {
		...
		case TOGGLE_MENU:
			if (action.menu.type === 'sub') {
				...
				open = state.open;
				const triggerIndex = state.trigger.findIndex((id) => id === action.menu.id);
		...
		case TOGGLE_FIXED_HEADER:
			return {
				...
				headerColor: !state.fixedHeader && initialState.headerColor === 'header-default' ? 'header-dark' : state.headerColor
			};
