...
export const CampaniasColumns: (
  ...
  (preSelectPol, preSelectIa, setIsAddFromIaPol) => {
    ...
    //HANDLES
    const handleRowSelectPol = async (numberCampaign: number, equipo: number, componente: number) => {
      const isSelected: boolean = !selectedCampaingsFromPol.includes(numberCampaign)
      await saveSelectedCamp(numberCampaign, isSelected, 'pol', equipo, componente);
    };

    const handleRowSelectIa = async (numberCampaign: number, equipo: number, componente: number) => {
      const isSelected: boolean = !selectedCampaingsFromIa.includes(numberCampaign)
      await saveSelectedCamp(numberCampaign, isSelected, 'ia', equipo, componente)
    };

    const saveSelectedCamp = async (
      numberCampaign: number,
      isSelected: boolean, type: 'pol' | 'ia',
      equipoId: number,
      componenteId: number
    ) => {
      setIsAddFromIaPol(true)
      await ax.post("service_render/campains/selected", { equipoId, componenteId, numberCampaign, isSelected, type })
        .then(() => {
          type === 'pol'
    ...
    const updateCampaingsPol = (numberCampaign: number, isSelected: boolean) => {
      if (isSelected) {
        setSelectedCampaingsFromPol([...selectedCampaingsFromPol, numberCampaign]);
      } else {
        setSelectedCampaingsFromPol(selectedCampaingsFromPol.filter(campaignSelected => campaignSelected !== numberCampaign));
      }
    };
    ...
    const updateCampaingsIa = (numberCampaign: number, isSelected: boolean) => {
      if (isSelected) {
        ...
        setSelectedCampaingsFromIa(selectedCampaingsFromIa.filter(campaignSelected => campaignSelected !== numberCampaign));
      }
    }
      ...
      },
      {
        ...
        cell: (campaign) => (
          <>
            <input type="checkbox"
              id={'selected_pol'}
              checked={selectedCampaingsFromPol.includes(campaign.numero_camp as number)}
              onChange={() => {
                handleRowSelectPol(
      ...
      },
      {
        ...
        cell: (campaign) => (
          <>
            <input type="checkbox"
              id={'selected_ia'}
              checked={selectedCampaingsFromIa.includes(campaign.numero_camp as number)}
              onChange={() => {
                handleRowSelectIa(
                  campaign.numero_camp as number,
                  campaign.equipo_id as number,
