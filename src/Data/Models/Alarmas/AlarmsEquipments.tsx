...
export const AlarmsEquipmentColumns: LocalizedColumnsCallback<
...
> = (intl) => {
  ...
  return [
                ...
                <Tooltip id={`tooltip-${randomUUID}`}>
                  {equipo.dataUsers.map((user) => (
                    <Fragment key={`alarms-eq-${randomUUID}`}>
                      <span>
                        {" "}
                        {`${caps(user.nombres)} ${caps(user.primer_apellido)}`}
                      </span>
                      <br />
                    </Fragment>
                  ))}
    ...
    },
    {
      ...
      center: true,
      format: (alarma) =>
        caps(alarma.is_active ? "labels:common.yes" : "labels:common.no"),
    },
  ];
};
