...
export const ApiSelectModal = <T extends unknown>(props: Props<T>) => {
  ...
  const searchByValue = async () => {
    ...
    if (props.pathToSearchByInput) {
      setLoadData(true);
      await ax
        .get(`${props.pathToSearchByInput}/${searchValue}`)
        .then((response) => {
      ...
      <div className="input-group">
        <div className="input-group-prepend border-left rounded-left">
          <i
            ...
            onClick={() => {
              if (
                (props.searchByInput && searchValue === "") ||
                !props.searchByInput
              ) {
                loadModal();
              }
        ...
        <input
          ...
          className={`btn form-control bg-white border rounded ${
            props.className ?? ""
          }`}
        ...
        {(value || display) && (
          <div className="input-group-append" style={{ cursor: "pointer" }}>
            <i
              className="far fa-lg fa-times-circle input-group-text bg-white text-danger"
              onClick={handleClear}
            />
          </div>
        )}
