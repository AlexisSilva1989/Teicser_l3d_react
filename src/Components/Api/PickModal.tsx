...
export const PickModal = <T extends unknown>(props: Props<T>) => {
	...
	const onChangeCallback = useCallback((row: T) => {
		modal.hide();
		setValue(row);
		if (onChange != null) {
			onChange(row);
	...
	useEffect(() => {
		...
		async function fetch() {
			...
			if (typeof props.source === 'string') {
				await api.get<T[]>(props.source, { params: params }).success(e => { data = e; }).fail('base.load');
			} 
			setSource(data);
	...
	}, [props.source, api, props.disabled, params, setLoading, props.default, onChangeCallback, initFetch, doInitFetch, props.defaultValue]);
	...
	useEffect(() => {
		...
		if(props.disabled) {
			...
			if(props.default != null) {
				...
				if(def != null) { onChangeCallback(def); }
				doInitDefault();
	...
	}, [initDefault, doInitDefault, source, onChangeCallback, props.default, props.disabled, props.defaultValue]);
	...
	return <div className='api-pick'>
		{props.name && <label><b>{input(props.name)}:</b></label>}
		<div className='input-group'>
			<div className='input-group-prepend api-pick-search'>
				<i className='fas fa-table input-group-text' style={props.disabled ? {} : { cursor: 'pointer' }}
					onClick={() => { if (!props.disabled) { modal.show(); } }} />
			</div>
			<input placeholder={props.placeholder ? placeholder(props.placeholder) : undefined}
				className={`btn form-control bg-white api-pick-input ${props.className ?? ''}`} style={props.disabled ? { cursor: 'default'} : { cursor: 'pointer' }}
				value={value ? props.displaySelector(value) : ''}
				readOnly name={props.id} disabled={props.disabled}
				onClick={() => { if (!props.disabled) { modal.show(); } }} />

				{value && (props.disabled == null || !props.disabled) && 
				<div className='input-group-append api-pick-clear' style={{ cursor: 'pointer' }}>
				<i className='fas fa-lg fa-times input-group-text bg-white text-danger' onClick={onClearCallback} />
