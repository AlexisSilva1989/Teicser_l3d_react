...
export const ItemsRequestUpload = (props: Props) => {
	...
	async function downloadExampleFile() {
		await ax.get($j(Links.SERVICES, 'solicitudes_articulos', 'ejemplo'), { responseType: 'blob' }).then((e: AxiosResponse) => {
	...
	return <>
		<div className='input-group'>
			...
			>
				<span className='input-group-text'><i className='text-danger text-center fas fa-trash' /></span>
			</div>
			...
			>
				<span className='input-group-text'><i className='text-success text-center fas fa-download' /></span>
			</div>
			...
			<div 
				...
				onClick={openFileSelect}
			>
				<span className='input-group-text'><i className='text-success text-center fas fa-upload' /></span>
			</div>			
		</div>
