...
const EquipOrderModal = <T extends unknown>({
  show,
  size = "sm",
  hide,
  title = "",
  ...
  return (
    <Modal show={show} size={size} onHide={hide} centered>
      ...
      <Modal.Body
        style={{ minHeight: "256px", maxHeight: "480px", overflowY: "scroll" }}
      >
        {isLoading ? (
          <LoadingSpinner />
        ) : (
          <div
            className="d-flex flex-column items-center justify-start"
            style={{ gap: 8 }}
          >
            ...
            <ReactSortable
              ...
              style={{ gap: 4 }}
              list={listEquip}
              setList={setListEquip}
              onEnd={() => setOrderDirection("desc")}
            >
              {listEquip.map((item) => (
                <div
                  key={item.id}
                  className="d-flex align-items-center bg-white px-3 py-3 border border-secondary rounded"
                  style={{ gap: 8 }}
                >
                  <i
      ...
      <Modal.Footer>
        {onSubmit && (
          <Button onClick={() => onSubmit && onSubmit(listEquip)}>
            {textButton}
          </Button>
        )}
      </Modal.Footer>
