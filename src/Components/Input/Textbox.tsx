...
export const Textbox = (props: Props) => {
	...
	function onKeyPressText(e: React.KeyboardEvent<HTMLInputElement>) {
		const val = e.currentTarget.value;
		setValue(val);
		if (props.onKeyPress) { props.onKeyPress(val); }
	}

	const name = props.name

	const nameIndustry = input(props.name+"."+localStorage.getItem("industria"))

	const outerClasses = ['msig-textbox'];
	if (props.className) { outerClasses.push(props.className); }
	const innerClasses = ['form-control border msig-textbox-input'];
	if (props.readonly) { innerClasses.push('readonly'); }
	if (props.inputOrientation) { innerClasses.push('text-'+props.inputOrientation); }

	return <div className={outerClasses.join(' ')}>
		{name && 
		<label><b>{props.industry ? nameIndustry : input(name)}:</b></label>}
		<div className='input-group'>
			<input type='text' className={innerClasses.join(' ')} name={props.id} maxLength={props.maxLength} readOnly={props.readonly} placeholder={props.placeholder}
				value={value}
