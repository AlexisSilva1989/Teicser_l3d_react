...
function LineGraph({
  ...
  const [dataSerie, setDataSerie] = useState<
    { name?: string; color?: string; type: string; data: serieData[] }[]
  >([
    {
      type: "line",
      color: primaryColor,
    ...
    chart: {
      ...
      events: {
        dataPointSelection: function (event, chartContext, config) {
          ...
          const dataPointSelected: serieData = (dataMedicion as serieData[])[
            dataPointIndex
          ];
          if (!dataPointSelected.usadoEnEntrenamiento) return;
          ...
          yaxis: {
            [0]: {
              ...
              label: {
                text: {
                  $set:
                    dataSelected?.y !== undefined &&
                    dataSelected?.x !== undefined
                      ? `${dataSelected?.date}: (${dataSelected.y?.toFixed(
                          1
                        )} mm - ${Number(dataSelected.x).toFixed(2)} MMTon)`
                      : undefined,
                },
              },
            },
            [1]: {
              y: { $set: dataSelected?.y },
              borderColor: {
                $set: dataSelected?.y ? selectedColor : "transparent",
              },
  ...
  useEffect(() => {
    let mediciones: serieData[] = showMediciones
      ...
      : (dataMedicion as serieData[]).filter(
          (medicion) => medicion.usadoEnEntrenamiento === false
        );

    let maxXMediciones = mediciones.reduce(
      (max, item) => (item.x > max ? item.x : max),
      -Infinity
    );
    let maxXData =
      dataLine.length > 0
        ? Number((dataLine[dataLine.length - 1] as serieData).x)
        : 0;
    setDataGraph((s) =>
      $u(s, {
        xaxis: {
          max: { $set: maxXData > maxXMediciones ? maxXData : maxXMediciones },
        },
      })
    );
