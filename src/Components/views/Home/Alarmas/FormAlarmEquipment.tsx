...
function FormAlarmEquipment({ dataInitial, isUpdate }: IFormAlarmEquipment) {
  ...
  const [notyfiableUsersSelected, setNotyfiableUsersSelected] = useState<
    string[] | undefined
  >([]);
  const [activeAlarms, setActiveAlarms] = useState<IAlarmsTypes>({
    critical_point: { isActive: false, value: null },
  });
  const [isReadyUsersNotifiable, setIsReadyUsersNotifiable] =
  ...
  const FormAlarmEquipmentSchema = yup.object().shape({
    ...
    notify_users: yup
      .array()
      .required("Seleccione almenos un (1) usuario a notificar"),
  });
  ...
  const onSubmit = async (data: IFormAlarmsEquipment) => {
    setIsSaving(true);
    isUpdate
      ? await updateAlarmsEquipment(data)
      : await postAlarmsEquipment(data);
  };
      ...
      .catch((e: AxiosError) => {
        if (e.response) {
          const message =
            ...
            caps("errors:base.post", { element: "alarma" });
  ...
  const updateAlarmsEquipment = (data: IFormAlarmsEquipment) => {
    ax.patch("equipments_alarms", data)
      .then(() => {
        goBack();
        addToast(caps("success:base.update"), {
          appearance: "success",
          autoDismiss: true,
        });
      })
      .catch((e: AxiosError) => {
        if (e.response) {
          const message =
            e.response.data.message ||
            caps("errors:update.", { element: "alarma" });
  ...
  useEffect(() => {
    const fetchNotyfiableUsers = async () => {
      ...
      await ax
        .get<Option<unknown>[]>("equipments_alarms/notyfiable_users")
        .then((response) => {
        ...
        .catch((e: AxiosError) => {
          if (e.response) {
            addToast(
              caps("errors:base.load", { element: "usuarios notificables" }),
              {
                appearance: "error",
                autoDismiss: true,
              }
  ...
  useEffect(() => {
    if (dataInitial !== undefined) {
      const dataUsers: string[] = dataInitial.dataUsers.map(
        ({ cusuario }) => cusuario
      );
      ...
      setActiveAlarms((state) =>
        $u(state, {
          critical_point: {
            isActive: { $set: dataInitial.is_alarm_critical_point },
          },
        })
      );
  ...
  return (
    <Form onSubmit={handleSubmit(onSubmit)}>
      ...
      <Col sm={12} className={"mb-3"}>
        <b>
          Tipos de notificaciones <span className="text-danger"> (*)</span>:
        </b>
      </Col>
      ...
      <Col>
        <Col className="alert alert-info mt-3">
          ...
          <strong>
            {" "}
            ¡Encienda las notificaciones que desea recibir del equipo!
          </strong>{" "}
          .
        </Col>
      </Col>
