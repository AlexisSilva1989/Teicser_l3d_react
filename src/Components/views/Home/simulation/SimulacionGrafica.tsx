...
const styleListOperationalVar: React.CSSProperties = {
  display: "flex",
  justifyContent: "center",
};

const SimulacionGrafica = ({
  resourceData,
  setFechaSimulacion,
  showLegend = true,
}: IProps) => {
      ...
      await ax
        ...
        .then((response) => {
          ...
          if (response.data.simulacion.length > 0) {
            setDataSimulacionSize(
              Object.keys(response.data.simulacion[0]).length
            );
            ...
            setFechaSimulacion &&
              setFechaSimulacion(response.data.fechaSimulacion);

            response.data.dataPromedio &&
              setDataPromedio(response.data.dataPromedio);
          }
        })
      ...
      { data: perfilCritico },
      {
        data:
          dataSimulacionSize > 0
            ? dataSimulacion[Object.keys(dataSimulacion)[rangeSelected]]
            : [],
      },
      { data: perfilNominal },
    ],
          ...
          <tbody>
            <tr>
              {dataPromedio?.TRAT_MOLINO && (
                <td key={"trat-molino"}>
                  <strong>Tonelaje: </strong> {dataPromedio.TRAT_MOLINO} Ton/día
                </td>
              )}
              {dataPromedio?.DWI && (
                <td key={"DWI"}>
              ...
              {dataPromedio?.BWI && (
                <td key={"BWI"}>
                  <strong>Dureza BWI: </strong> {dataPromedio.BWI}
                </td>
              )}
            </tr>
            <tr>
              {dataPromedio?.BOLAS_TON && (
                <td key={"BOLAS_TON"}>
                  <strong>Cargío bolas: </strong> {dataPromedio.BOLAS_TON}{" "}
                  Ton/día
                </td>
              )}
              {dataPromedio?.AI && (
              ...
              {dataPromedio?.PH && (
                <td key={"PH"}>
                  <strong>PH: </strong> {dataPromedio.PH}
                </td>
              )}
            </tr>
            <tr>
              {dataPromedio?.VEL_RPM && (
                <td key={"PH"} colSpan={3}>
                  <strong>Velocidad: </strong> {dataPromedio.VEL_RPM} RPM
                </td>
              )}
            </tr>
          </tbody>
          ...
          <>
            <Col sm={12} className="mb-2 p-0">
              <Col className="alert alert-info mb-0" sm={12}>
                ...
                Periodo seleccionado para realizar la proyección de variables es
                de
  ...
  const graph: JSX.Element = (
    <>
      <Col className="col-md-6">
        ...
        {dataSimulacion !== undefined &&
          Object.keys(dataSimulacion).length > 0 && (
            <Form.Group as={Row}>
              ...
              <Col sm="6">
                <Form.Control
                  ...
                  type="range"
                  min={0}
                  max={dataSimulacionSize - 1}
                  onChange={(e) => setRangeSelected(Number(e.target.value))}
                />
                <h4 className="text-center">
                  {Object.keys(dataSimulacion)[rangeSelected]}
                </h4>
              </Col>
              <Form.Label column sm="3">
                {
                  Object.keys(dataSimulacion)[
                    Object.keys(dataSimulacion).length - 1
                  ]
                }
              </Form.Label>
            </Form.Group>
  ...
  const messagesDataNoReady: { [key: string]: string } = {
    PENDIENTE:
      "La simulación se encuentra en cola para ser realizada, por favor espere",
  ...
  const ShowModule: JSX.Element = (
    <>
      {dataSimulacionSize > 0 ? (
        <>
          <Col xl="12" className="p-0">
            <Col sm={12} className="text-center mb-4">
              <h4>{`${equipo} (${tipoEquipo}) / ${component} `}</h4>
            </Col>
            {graph}
        ...
        <>
          {(statusSimulation === "PENDIENTE" ||
            statusSimulation === "EN PROCESO") && <LoadingSpinner />}
          <ShowMessageInModule
            message={
              statusSimulation !== undefined &&
              messagesDataNoReady.hasOwnProperty(statusSimulation)
...
function CustomTooltip({ getStyle, primaryAxis, datum }: any) {
  ...
  return datum ? <>Espesor: {primaryAxis.format(datum.secondary)}</> : null;
}
