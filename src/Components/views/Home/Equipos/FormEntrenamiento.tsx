...
const FormEntrenamiento = ({
  ...
  const { handleSubmit, control, errors, setValue, register } =
    useForm<IDataFormEntrenamiento>({
      mode: "onSubmit",
  ...
  const setPerfilNominal = async (file: File) => {
    ...
    let sheetValues: object[] = $x.utils.sheet_to_json(
  ...
  const updateComponentes = async (equipoId: string) => {
    await ax
      .get<{ id: string; nombre: string }[]>(
        "service_render/equipos/componentes_entrenar",
                  ...
                  rules={{
                    validate: {
                      value: (value) => {
                        ...
                        if (value && value[0]) {
                          let primeraColumna = value[0][0]
                            .toString()
                            .toLowerCase();
                          let segundaColumna = value[0][1]
                            .toString()
                            .toLowerCase();
                          isValueCorrect =
                            primeraColumna == "coordenada_x" &&
                            segundaColumna == "coordenada_y";
                        }
                        return (
                          isValueCorrect ||
                          "Cabecera de primera columna como (coordenada_x) y segunda columna como (coordenada_y)"
                        );
                      },
                    },
                  }}
