...
const FormEquipo = ({
  ...
  const [componentSelected, setComponentSelected] = useState<
    string[] | undefined
  >([]);
  const [componentAvailable, setComponentAvailable] = useState<
    Option<string>[]
  >([]);
  ...
  const getServesEquipo = async (equipoId: string | undefined) => {
    await ax
      .get<{ serversSelected: string[]; serversAvailable: Option<string>[] }>(
        "service_render/equipos/servidores",
        { params: { equipoId: equipoId } }
      )
      .then((response) => {
        setServerSelected(response.data.serversSelected);
        setServerAvailable(response.data.serversAvailable);
        setValue([{ server_selected: response.data.serversSelected }]);
      })
      .catch((e: AxiosError) => {
        if (e.response) {
  ...
  const getComponentsEquipo = async (equipoId: string | undefined) => {
    await ax
      .get<{
        ...
        componentsAvailable: Option<string>[];
      }>("service_render/equipos/componentes", {
        params: { equipoId: equipoId },
      })
      .then((response) => {
        setComponentSelected(response.data.componentsSelected);
        setComponentAvailable(response.data.componentsAvailable);
        setValue([{ components_selected: response.data.componentsSelected }]);
      })
      .catch((e: AxiosError) => {
        if (e.response) {
                ...
                validate: {
                  value: (value: string[]) => {
                    return (
                      ...
                      "Seleccione almenos un (1) componente"
                    );
                  },
                },
              }}
            ...
            {isEdit && (
              <Col className="alert alert-warning mt-3">
                <i
                  ...
                  aria-hidden="true"
                />
                Remover un componente seleccionado para el equipo, elimina los
                datos de entrenamiento existente para el mismo
              ...
              rules={{
                validate: {
                  value: (value: string[]) => {
                    return (
                      (value != undefined && value.length > 0) ||
                      "Seleccione almenos un (1) servidor"
                    );
                  },
                },
              }}
