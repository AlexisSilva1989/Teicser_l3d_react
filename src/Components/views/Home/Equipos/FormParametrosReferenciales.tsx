...
const FormParametrosReferenciales = ({ onSubmit, isSaving, initialData }: IProps) => {

  const { capitalize: caps } = useFullIntl();
  const { handleSubmit, errors, setValue, register, control, watch } = useForm<IParametrosReferencias>({
    mode: "onSubmit",
  ...
  return (<><form onSubmit={handleSubmit(onSubmit)} noValidate>
    <Row>
      ...
      <Col sm={12} className="pt-2">
        <Col sm={12} className="px-0 border rounded">
          <Col className="pt-3">
            <p className="mb-0" style={{ fontSize: "12px", fontWeight: 600 }}>Configuración de IA</p>
          </Col>
          <hr />
          <Col sm={2}>
            <label><b>Proyectar:</b></label>
          ...
          <Col className="px-0">
            ...
            <hr />
            <Col sm={4}>
              <label><b>Periodo a considerar:</b></label>
              <Controller control={control}
                ...
                rules={{ required: { value: isIa === "1", message: caps('validations:required') } }}
                style={{ display: "inline" }}
                disabled={isIa !== "1"}
                as={RadioSelect}
              />
