...
export const SelectAdd = <T extends unknown>(props: Props<T>) => {
  ...
  const formatCreateLabel = (inputValue: string) =>
    `${props.placeholderAddElement ?? `Crear item`}  ${inputValue}`;
  ...
  useEffect(() => {
    ...
    if (onChange != null) {
      if (props.value != null) {
        ...
        if (props.isSelectFirstOption && state.data.length > 0) {
    ...
    if (typeof props.source === "string") {
      ...
      if (result == null || result.data == null) {
        ...
        const d =
          filter == null
            ? result.data
            : result.data.filter((x) => filter(selector(x)));
        setState((s) => $u(s, { $merge: { data: d ?? [] } }));
        if (d.length > 0 && onChange != null && props.value == null) {
          if (
            props.isSelectFirstOption === undefined ||
            props.isSelectFirstOption
          ) {
  ...
  return (
    <div
      className={
        "ServerSelect form-group " + (props.span ? "col-" + props.span : "")
      }
    >
      {props.label && (
        <label>
