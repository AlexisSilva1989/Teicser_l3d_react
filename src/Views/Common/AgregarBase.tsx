...
export const AgregarBase = <TRaw extends unknown, TData extends unknown>(props: Props<TRaw, TData>) => {
	...
	async function onSubmit(data: any) {
		...
		const tieneImagen = props.fields.some( field => field.type === FILE_INPUT_PREVIEW );
		const headers = tieneImagen ? { headers: { "Content-Type": "multipart/form-data" } } : { "Content-Type": "application/json" }  ;
		...
		await ax.post(props.path, tieneImagen ? formData : data , headers)
			.then((response) => {
				if(response.status === 202){
					addToast(caps('errors:base.post', { element: localize(props.warningElement) }), {
						appearance: 'error',
						autoDismiss: true,
					});
			...
			.catch((e: AxiosError) => {
				if (e.response) {
					const messageError = e.response.data.hasOwnProperty('errors') ? e.response.data.errors[Object.keys(e.response.data.errors)[0]] 
						: caps('errors:base.post', { element: localize(props.errorElement) });
