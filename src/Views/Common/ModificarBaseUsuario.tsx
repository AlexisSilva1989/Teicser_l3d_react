...
import React, { useState, useEffect, ReactNode, useMemo, Fragment } from 'react';
import { $u, $j } from '../../Common/Utils/Reimports';
import { Redirect } from 'react-router-dom';
import { FormBase } from './FormBase';
import { ValidatedInput, EMBEDDED, SELECT_MODAL } from '../../Components/Forms/ValidatedForm';
...
export const ModificarBaseUsuario = <TCast extends unknown, TRaw extends unknown, TData extends unknown>(props: Props<TCast, TRaw, TData>) => {
	...
	const fields = useMemo(() => {
		const elements = props.fields.map((x) => {
			if (x.type === 'EMBEDDED_VALUE') {
				...
				return {
					...x,
					initialValue: x.select == null || data.element == null ? undefined : x.select(data.element)
				};
			}
			return {
				...x,
				value: x.select == null || data.element == null ? undefined : x.select(data.element)
			};
		});
		// const foo = '';
	...
	}, [element, props.path, props.errorElement, getId, caps, onChangeData, localize]);
	...
	return (
		<Fragment>
			<FormBase<TRaw, TData> title={props.title} onSubmit={onSubmit} loading={loading} onSerialize={props.onSerialize} onReset={props.onReset} validations={props.validations} fields={fields as any} />
			<Modal show={modalInactividad.visible} onHide={modalInactividad.hide}>
				...
				<Modal.Footer className='text-right'>
					<Button variant='outline-secondary' className='mr-3' onClick={modalInactividad.hide}>
						{label('cancel')}
					</Button>
