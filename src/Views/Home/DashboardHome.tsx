...
export const DashboardHome = () => {
  ...
  const [equiptmentsByType, setEquiptmentsByType] =
    useState<EquiposPorTipo[]>();
  const [loadingData, setLoadingData] = useState(false);

  const SAGEquipments =
    equiptmentsByType?.find((type: any) => type.nombre_corto === "SAG")
      ?.equipos ?? [];
  const BALLEquipments =
    equiptmentsByType?.find((type: any) => type.nombre_corto === "MOBO")
      ?.equipos ?? [];
  ...
  return loadingData ? (
    ...
    <BaseContentView asDiv={true}>
      <div className="h-100 d-flex flex-column justify-content-center">
        <Row
          className={`w-100 m-0 d-flex flex-nowrap ${
            SAGEquipments.length > 3 && "overflow-x-scroll"
          }`}
        >
          {SAGEquipments.map(({ nombre: title, id }, index) => (
            <Col sm="12" md="4" className="">
              <Card
                onClick={() => handleMillCardClick(id)}
                key={index}
                className="mill-card px-2 py-4 text-center d-flex flex-column align-items-center"
              >
        ...
        <Row
          className={`w-100 m-0 d-flex flex-nowrap ${
            BALLEquipments.length > 4 && "overflow-x-scroll"
          }`}
        >
