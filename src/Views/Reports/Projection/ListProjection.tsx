...
import {
  Projection,
  ProjectionsColumns,
} from "../../../Data/Models/Gemelo/Projection";
...
export const ListProjection = () => {
  ...
  const updateComponentes = async (equipoId: string) => {
    ...
    setFiltersParams((state) =>
      $u(state, { filterByComponente: { $set: undefined } })
    );
  };

  return (
    <ListaBase<Projection>
    ...
    >
      ...
      <ApiSelect<IComponente>
        ...
        value={
          filtersParams.filterByComponente == undefined
            ? "-1"
            : filtersParams.filterByComponente
        }
        ...
        onChange={(data) => {
          setFiltersParams((state) =>
            $u(state, {
              filterByComponente: { $set: data != "-1" ? data : undefined },
            })
          );
        }}
        firtsOptions={{ label: "TODOS", value: "-1" }}
      ...
      <ApiSelect
        ...
        value={
          filtersParams.filterByPeriodo == undefined
            ? "-1"
            : filtersParams.filterByPeriodo
        }
        source={[
          { label: "TODOS", value: "-1" },
        ...
        onChange={(data) => {
          setFiltersParams((state) =>
            $u(state, {
              filterByPeriodo: { $set: data != "-1" ? data : undefined },
            })
          );
        }}
      />
      <ApiSelect
        ...
        value={
          filtersParams.filterByStatus == undefined
            ? "-1"
            : filtersParams.filterByStatus
        }
        source={[
          { label: "TODOS", value: "-1" },
        ...
        onChange={(data) => {
          setFiltersParams((state) =>
            $u(state, {
              filterByStatus: { $set: data != "-1" ? data : undefined },
            })
          );
        }}
      />
