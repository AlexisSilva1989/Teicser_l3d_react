...
import FormProjection, {
  IdataFormProjection,
  IDataPromedio,
  IDatesLastProjection,
} from "../../../Components/views/Home/Projection/FormProjection";
...
export const AddProjection = () => {
  ...
  const [datesLastProjection, setDatesLastProjection] =
    useState<IDatesLastProjection>();
  const [lastDateProjection, setLastDateProjection] = useState<string>();
  const [dataForm, setDataForm] = useState<IdataFormProjection>();
  const [daysProjection, setDaysProjection] = useState<string>(
    countDaysToProjection.toString()
  );
  const [dateFillEnd, setDateFillEnd] = useState<string>();
  const [typeProjection, setTypeProjection] =
    useState<string>("projection30Days");
  ...
  const [datesSampling, setDatesSampling] = useState<{
  ...
  const handleSubmitProjection = async (data: IdataFormProjection) => {
    ...
    await api
      .post("service_render/projection_operational_var", data)
      .success((response) => {
        goBack();
        setLoading(false);
      })
      .fail("No se pudo consumir el servicio", null, () => {
        setLoading(false);
      });
  };
  ...
  const onChangeTypeProjection = (typeProjection: string) => {
    if (dataForm) {
      dataForm.type_projection = typeProjection;
    }
  ...
  const updateLastDataSimulate = (
    historial_data_render: IDatesLastProjection
  ) => {
    setDatesLastProjection(historial_data_render);
    setLastDateProjection(
      $m(historial_data_render.fecha_medicion, "YYYY-MM-DD").format(
        "DD-MM-YYYY"
      )
    );
  };
  ...
  useEffect(() => {
    const getLastDataSimulated = async () => {
      ...
      await api
        .get<responseInfoRender>($j("service_render/get_last_data_simulated"), {
        ...
        .success((response) => {
          ...
          response.historial_data_render &&
            response.historial_data_render.status === "ERROR" &&
            errors.push(
              "Ha ocurrido un error en el último registro de carga de variables operacionales para este componente, por favor realice nuevamente la carga de datos operacionales"
            );

          response.historial_data_render &&
            (response.historial_data_render.status === "PENDIENTE" ||
              response.historial_data_render.status === "EN PROCESO") &&
            errors.push(
              "La carga de variables operacionales del componente esta pendiente, por favor espere que termine "
            );
          ...
          response.dates_sampling.start === null
            ? errors.push("No se ha encontrado fecha de sampling inicial") &&
              setDatesSampling((state) =>
          ...
          response.dates_sampling.end === null
            ? errors.push("No se ha encontrado fecha de sampling final") &&
              setDatesSampling((state) =>
            ...
            : setDatesSampling((state) =>
                $u(state, {
                    ...
                    $set: $m(response.dates_sampling.end, "YYYY-MM-DD").format(
                      "DD-MM-YYYY"
                    ),
                  },
                })
        ...
        .fail("Error al consultar los datos de simulación", () => {
          ...
          errors.push(
            "Se ha producido un error obteniendo los datos asociados a la proyección"
          );
        })
        .finally(() => {
  ...
  useEffect(() => {
    if (lastDateProjection !== undefined && dateFillEnd !== undefined) {
      setDaysProjection(
        DateUtils.differenceBetweenDates(
          lastDateProjection,
          dateFillEnd
        ).toString()
      );
  ...
  useEffect(() => {
    lastDateProjection !== null &&
      lastDateProjection !== undefined &&
      setDateFillEnd(
        $m(lastDateProjection, "DD-MM-YYYY")
          .add(countDaysToProjection, "days")
          .format("DD-MM-YYYY")
      );
  }, [lastDateProjection]);
