...
function ShowSimulationSummary() {
  ...
  const dataStateAs =
    stateAs<{ data: { id: number; fecha_simulacion: string } }>();
  ...
  const [simulationsComponents, setSimulationsComponents] =
    useState<JSONObject>();
  ...
  useEffect(() => {
    async function feat() {
      await ax
        .get<{
          simulaciones: JSONObject;
          equipo_nombre: string;
          equipo_tipo: string;
        }>($j("service_render/summary_projection_equipment", idEquipoSelected!))
        .then((response) => {
