...
export const IndexImagenCondicion = () => {
  ...
  const pdfDescargar = (pdf: any) => {
    setLoading(true);
    api
      .get<string | Blob | File>($j("img_descargar", pdf.ruta.toString()), {
        responseType: "blob",
      })
      .success((e) => {
        $d(e, pdf.pdf_name);
  ...
  const imgEliminar = async (img: any) => {
    api
      .get<any>($j("delete_img", img.id.toString()))
      .success((e) => {
        setLoading(false);
        doReloadTable();
        addToast(caps("success:base.success"), {
    ...
    cell: (img) => (
      <>
        <Col sm={6}>
          ...
          >
            <i
              className="fas fas fa-download"
              style={{ cursor: "pointer", color: "#09922C" }}
              onClick={() => pdfDescargar(img)}
            />
          </OverlayTrigger>
        </Col>
        <Col sm={6}>
          <OverlayTrigger
            placement="top"
            overlay={<Tooltip id={`tooltip-1`}>Eliminar</Tooltip>}
          >
            <i
              className="fas fas fa-trash-alt"
              style={{ cursor: "pointer", color: "#09922C" }}
              onClick={() => imgEliminar(img)}
            />
          </OverlayTrigger>
        </Col>
      </>
  ...
  const onClickEnviar = async () => {
    ...
    setLoading(true);
    await ax
      .post("img_save", formData, headers)
      .then((response) => {
        addToast(caps("success:base.success"), {
          appearance: "success",
          autoDismiss: true,
