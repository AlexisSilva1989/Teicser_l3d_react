...
import {
  IVariables,
  SagOperationalColumns,
  SagOperationalDataImport,
} from "../../../Data/Models/Render/SagOperationalData";
import {
  mobo_data_structure,
  sag_data_structure,
...
import {
  MoboOperationalColumns,
  MoboOperationalDataImport,
} from "../../../Data/Models/Render/MoboOperationalData";
...
export const IndexOperationalData = () => {
  ...
  const [operationalData, setOperationalData] = useState<
    (SagOperationalDataImport | MoboOperationalDataImport)[]
  >([]);
  ...
  const handleChangeFile = async (file: File | null) => {
    ...
    if (file) {
      dataDocument = await FileUtils.readFileToArray<
        SagOperationalDataImport | MoboOperationalDataImport
      >(file, 0, "A:AE10000", getStructureEquipo());
  ...
  const validateStructureFile = (header: string[]) => {
    ...
    setErrorsInStructureFile(
      errosInStructureFile.length > 0 ? [errosInStructureFile.join(", ")] : []
    );
  };

  const mappedDataForDate = (
    FileReception: (SagOperationalDataImport | MoboOperationalDataImport)[]
  ) => {
  ...
  const ExcelDateToJSDate = (
    data: SagOperationalDataImport | MoboOperationalDataImport,
    index: number
  ) => {
  ...
  const onSimulateProjection = async () => {
    ...
    setLoading(true);
    await api
      .post("service_render/render_data", formData, headers)
      .success((response) => {
  ...
  const descargarEjemplo = () => {
    setLoading(true);
    api
      .get<string | Blob | File>($j("ejemplo_variables"), {
        responseType: "blob",
      })
      .success((e) => {
  ...
  const updateComponentes = async (equipoId: string) => {
    await ax
      .get<IComponente[]>("service_render/equipos/componentes_entrenados", {
  ...
  useEffect(() => {
    ...
    const fechaMedicion = operationalData.reduce(
    ...
    if ($m(fechaMaxima).isValid() && $m(fechaMedicion).isValid()) {
      ...
      setVariables((state) =>
        $u(state, {
          ...
          samplingDatesStart: {
            $set: $m(fechaMedicion).subtract(30, "days").format("DD-MM-YYYY"),
          },
  ...
  useEffect(() => {
    ...
    if (fillDatesStart.isValid() && xampleDateEnd.isValid()) {
      setVariables((state) =>
        $u(state, {
          isRandomSampling: {
            $set: fillDatesStart.isBefore(xampleDateEnd) ? "true" : "false",
          },
        })
      );
    }
  ...
  useEffect(() => {
    variable.csv_import !== null &&
      variable.csv_import !== undefined &&
      handleChangeFile(variable.csv_import);
  }, [tipoEquipoSelected]);
      ...
      <Row className="mb-3">
        ...
        <Col sm={12}>
          <Col className="alert alert-info mt-2">
            ...
            Fechas (sampling) que seran tomadas en cuenta para el entrenamiento
          </Col>
        </Col>
      </Row>
        ...
        <Modal.Body>
          ...
          <Row className="mb-3">
            ...
            <Col sm={12}>
              <Col className="alert alert-info mt-2">
                <i className="fa fa-info mr-2" aria-hidden="true" />
                Representan el rango de fechas que serán proyectados.
              </Col>
            </Col>
          </Row>
        ...
        {loadingData ? (
          ...
          <>
            <Col className="d-flex justify-content-between align-items-end pr-0 pl-0">
              <Col sm={6}>
                <h5>
                  {tipoEquipoSelected && `Molino tipo: ${tipoEquipoSelected}`}
                </h5>
                <h5>
                  {variable.xampleDateStart &&
            ...
            <Col sm={12}>
              {loadingg ? (
                ...
                <Col className="alert alert-warning mt-3 text-center">
                  <i
                    className="fa fa-exclamation-triangle fa-4x m-3"
                    aria-hidden="true"
                  />
                  <h3>Error en estructura de archivo</h3>
                  <h5>
                    El archivo que ha sido cargado no cumple con la estructura
                    esperada, por favor verifique las siguientes posiciones en
                    la cabecera:{" "}
                  </h5>

                  {errorsInStructureFile.map((error, index) => (
                    <span key={`structure-error-${index}`}>{error}</span>
                  ))}
                </Col>
              ) : (
                <ApiTable<any>
