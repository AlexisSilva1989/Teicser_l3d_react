...
export const AdministrarRoles = () => {
	...
	const [dataOfChangeOfPermission, setDataOfChangeOfPermission] = useState(initial.dataOfChangeOfPermission);
	...
	return (
		<BaseContentView title='titles:roles_management'>
			<Container>
				<Row>
					<Col className='d-flex justify-content-start'><Buttons.Add onClick={onClickAgregarRol} /></Col>
				</Row>
				<br></br>
				<Row>
					{data.roles && !reload ? (
									...
									<tr>
										<td id='modulo'>MÓDULOS</td>
										{
											data.roles.map( (rol , i) =>{
												return (
													<td id='modulos' key={i}>{rol.nombre}</td>
												) 	
											})
										}
									</tr>
															...
															<td 
																...
																onClick={ () => { 
																	handleClickModulePermissions( {
																		...
																		roleId: role.codigo,
																		entityName: entity.nombre.replace("_" , " "),
																		permission: permission,
																		entityId: entity.id
																	} ) 
																}} 
															> 
																{ permission === 0 ? <i className='fas fa-eye-slash' />  : "" } 
																{ permission === 1 ? <i className='fas fa-eye' />  : "" } 
																{ permission === 2 ? <i className='fas fa-plus-circle' />  : "" } 
																{ permission === 3 ? <i className='fas fa-edit' />  : "" } 
																{ permission === 4 ? <i className='fas fa-trash-alt' />  : "" } 
															</td>
														)
													})
												}
						...
						<BounceLoader color='var(--primary)' css={{ margin: '2.5rem auto' } as any} size={64} />
					)}
				</Row>
			</Container>
			...
			<div>
				<Modal title='titles:add_role' ref={modalAgregar} onClose={() => setResetAgregar(() => true)}>
					<div className='modal-body'>
						<ValidatedForm
							validations={{
								nombre: {
