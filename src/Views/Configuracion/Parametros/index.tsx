...
const Parametros = () => {
	...
	const [cambios, setCambios] = useState<Record<| 'valor' | 'descripcion', string>>();
	...
	function persistirCambiosParametro() {
		ax.patch(joinUrls('v2', 'parametros', seleccionado!.id.toString()), cambios).then(() => {
			const index = parametros.findIndex(parametro => parametro === seleccionado);
			setParametros(state => $u(state, { [index]: { $set: { ...parametros[index], ...cambios } } }));
			doReload();
		}).finally(() => {
	...
	return <ContainerView title='parameters'>
		...
		<Col sm={12}>
	        {loadingg ? (
			    ...
			    <ListaParametros source={parametros} reload={reload} onSelect={abrirModal} />
			)}
		</Col>
		...
		<ModificarParametro parametro={seleccionado} onSubmit={modificarParametro} visible={modalModificar.visible} hide={modalModificar.hide} />
		<ConfirmarModificarParametro onSubmit={persistirCambiosParametro} visible={modalPersistirCambios.visible} hide={modalPersistirCambios.hide} />
	</ContainerView>;
};
