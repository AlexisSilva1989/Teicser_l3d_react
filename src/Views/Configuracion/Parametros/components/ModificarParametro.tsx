...
const schemaString = (msg: (value: string, args?: any) => string) => yup.object().shape({
	valor: yup.string().required(msg('required')).min(1, msg('min_length', { length: 1 })),
});

const schemaFloat = (msg: (value: string, args?: any) => string) => yup.object().shape({
	valor: yup.number()
...
const schemaInteger = (msg: (value: string, args?: any) => string) => 	yup.object().shape({
	valor: yup.number()
		.typeError(msg('numeric_field', {field: 'value'}))
		.required(msg('required'))
...
const schemaTime = (msg: (value: string, args?: any) => string) => yup.object().shape({
...
export const ModificarParametro = (props: Props) => {
	...
	const { handleSubmit, register, errors, control, setValue } = useForm({ validationSchema:  schema});
	...
	const inputTypeParams = () => {
		const elementParamsType = ["double","integer","string"].includes(parametro!.tipoDato) ? "TEXT"
			:parametro!.tipoDato.toUpperCase();

		const typeInput: { [key: string]: any } = {
			"TEXT": <>
				<Form.Control type='text' name='valor' defaultValue={parametro!.valor} ref={register}
					onChange={(event: React.ChangeEvent<HTMLInputElement>) => {
						if(parametro?.tipoDato === "integer"){
							event.target.value = event.target.value.replace(ONLY_NUMBER_INTEGER, '');
						}else if(parametro?.tipoDato === "double"){
							event.target.value = event.target.value.replace(ONLY_NUMBER, '');
			...
			"BOOLEAN": <>
				...
				<Form.Check ref={register}
					...
					onChange={() => { setCheckValue("false") }}

				/>
			</>,
			"TIME":<>
				<Controller as={DatePickerController} control={control} name="valor"/>
			</>
		};
	...
	if (props.parametro == null) { return null; }
		...
		<Form onSubmit={handleSubmit(onSubmit)}>
			...
			<Modal.Body>
				<Form.Group>
					...
					<Form.Control type='text' name='descripcion' readOnly={true} defaultValue={parametro!.descripcion} ref={register} />
