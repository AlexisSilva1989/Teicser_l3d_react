...
export const Login = () => {
	...
	const doLogin = useCallback((user: string, password: string) => dispatch(login(user, password)), [dispatch]);

	function onSubmit(e: { nombre_usuario: string, clave: string }) {
		doLogin(e.nombre_usuario, md5(e.clave));
	}

	return <BaseLoginComponent >
		<ValidatedForm submitIcon='' submitLabel='labels:links.login' onSubmit={onSubmit}
			validations={{
				nombre_usuario: {
					presence: {
						allowEmpty: false,
		...
		/>
		<Link  to="/restablecer_contraseña">{caps('labels:links.forgot_my_password')}</Link>
	</BaseLoginComponent>
