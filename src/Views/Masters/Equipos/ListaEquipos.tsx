...
import { EquipoColumns, EquipoTipo, ItemOrderEquip, tiposEquipos, tiposEquiposWithAll } from '../../../Data/Models/Equipo/Equipo';
...
export const ListaEquipos = () => {
	...
	const getList = useCallback(async () => {
		...
		await ax.get($j('service_render/equipos'), { params: { showStatus: true } })
			.then((response) => {
	...
	const saveOrderedList = useCallback(async (order: { id: string | number, order: number }[]) => {
		setIsSavingOrder(true);
		await ax.post($j('service_render/equipos'), { order })
			.then((response) => {
				getList();
				doReload();
				orderModal.hide();
	...
	const submitOrderEquip = useCallback(
		async (data: ItemOrderEquip[]) => {
			const mappedData = data.map((item, index) => ({ id: item.id, order: index }))
	...
	return (<>
		...
		<EquipOrderModal<EquipoTipo[]> show={orderModal.visible} title="Orden de Equipos" hide={orderModal.hide} initialState={equipList.map(item => ({ id: item.id, name: item.nombre }))} onSubmit={submitOrderEquip} isLoading={isSavingOrder} />
	</>);
};
