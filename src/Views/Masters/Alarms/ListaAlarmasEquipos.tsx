...
import { AlarmsEquipmentColumns, extendColumnsAlarms, IFormAlarmsEquipment } from '../../../Data/Models/Alarmas/AlarmsEquipments'
...
function ListaAlarmasEquipos() {
  ...
  const customFilter = useCallback((equipo: IFormAlarmsEquipment): boolean => {
    if (filter.active === '-1') {
      return true;
    }
      ...
      >
        <Col>
          ...
          <CustomSelect
            ...
            onChange={(e) => { setFilter((s) => $u(s, { active: { $set: e.value } })) }}
            options={[
