...
const ListaFabricantes = () => {
  ...
  const customFilter = useCallback(
    (fabricante: Fabricante): boolean => {
      const isFilterByStatus: boolean =
        filter.status == "-1" || fabricante.status == filter.status;
  ...
  return (
    <>
      ...
      <FabricanteFormModal
        show={modalManufacturers.visible}
        hide={modalManufacturers.hide}
        size="sm"
        ...
        initialState={manufacturerSelected}
      />
    </>
  );
};
