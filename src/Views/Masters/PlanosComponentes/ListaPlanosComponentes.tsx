...
const ListaPlanosComponentes = () => {
  ...
  const customFilter = useCallback(
    (componentesPlano: IComponentesPlano): boolean => {
      const isFilterByStatus: boolean =
        filter.status == "-1" || componentesPlano.status == filter.status;
  ...
  const onSubmitEdit = async (data: any) => {
    ...
    setIsSaving(true);
    await ax
      .patch("componentes_planos", formData, headers)
      .then((response) => {
        modalPlanosComponentes.hide();
        doReload();
        addToast(caps("success:base.save"), {
  ...
  return (
    <>
      ...
      <ComponentesPlanoFormModal
        show={modalPlanosComponentes.visible}
        hide={modalPlanosComponentes.hide}
        size="sm"
