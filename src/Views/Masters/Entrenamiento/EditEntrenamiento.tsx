...
import {
  IDataFormEntrenamiento,
  IEntrenamiento,
} from "../../../Data/Models/Entrenamiento/Entrenamiento";
...
export const EditEntrenamiento = () => {
  ...
  const [trainingSelected, setTrainingSelected] =
    useState<IDataFormEntrenamiento>();
  ...
  const handleSubmit = async (data: IDataFormEntrenamiento) => {
    ...
    data.nominal_file &&
      formData.append("nominal_data", JSON.stringify(data.nominal_file));
    data.critical_file &&
      formData.append("critical_data", JSON.stringify(data.critical_file));
    data.model_file && formData.append("model_file", data.model_file);
    data.scaler_file && formData.append("scaler_file", data.scaler_file);

    setIsSaving(true);
    await ax
      .patch("service_render/entrenamiento/edit", formData, headers)
      .then((response) => {
        goBack();
  ...
  return (
    <BaseContentView title="Modificar entrenamiento">
      ...
      <div className="col-12 mb-3">
        <Row>
          <Col sm="12">
            <b>Equipo: </b> {trainingSelected?.equipo}
          </Col>
          <Col sm="12" className="mt-3">
            <b>Componente: </b> {trainingSelected?.componente}{" "}
          </Col>
        </Row>
        <FormEntrenamiento
