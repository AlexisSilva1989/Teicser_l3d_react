...
export const AddEntrenamiento = () => {
  ...
  const handleSubmit = async (data: IDataFormEntrenamiento) => {
    ...
    data.nominal_file &&
      formData.append("nominal_data", JSON.stringify(data.critical_file));
    data.critical_file &&
      formData.append("critical_data", JSON.stringify(data.critical_file));

    setIsSaving(true);
    await ax
      .post("service_render/entrenamiento/save", formData, headers)
      .then((response) => {
        goBack();
  ...
  return (
    <BaseContentView title="Agregar ">
      <div className="col-12 mb-4">
        <Buttons.Back />
      </div>
      <div className="col-12 mb-3">
        <FormEntrenamiento onSubmit={handleSubmit} isSaving={isSaving} />
      </div>
    </BaseContentView>
  );
};
