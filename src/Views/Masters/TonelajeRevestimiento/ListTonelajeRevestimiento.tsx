...
import {
  ITonelajeRevestimientosTable,
  TonelajeRevestimientosColumns,
} from "../../../Data/Models/TonelajeRevestimientos/TonelajeRevestimientos";
...
function ListTonelajeRevestimiento() {
  ...
  const downloadExcel = async (equipment: string) => {
    setLoading(true);
    const nombreArchivo = `TDR_${equipment.toUpperCase()}_${$m().format(
      "YYYYMMDDHHmmss"
    )}.xlsx`;

    await ax
      .get($j("service_render", "tdr", "excel"), {
        responseType: "blob",
        params: { equipment },
      })
      ...
      <BaseContentView title="titles:tdr">
        ...
        <Col sm={12} className="mt-3">
          <ApiTable<ITonelajeRevestimientosTable>
            columns={TonelajeRevestimientosColumns(intl, {
              onClickExcel: downloadExcel,
            })}
