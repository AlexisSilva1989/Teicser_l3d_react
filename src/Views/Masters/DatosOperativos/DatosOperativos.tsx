...
import {
  CampaniasColumns,
  ICampania,
} from "../../../Data/Models/Campanias/Campanias";
...
export default function DatosOperativos() {
  ...
  const [reloadTableImportDataProfiles, doReloadTableImportDataProfiles] =
    useReload();
  const [reloadTableImportDataCampaigns, doReloadTableImportDataCampaigns] =
    useReload();
  ...
  const [idEquipoSelected, setIdEquipoSelected] = useState<
    string | undefined
  >();
  const [nombreEquipoSelected, setNombreEquipoSelected] = useState<
    string | undefined
  >();
  const [tipoEquipoSelected, setTipoEquipoSelected] = useState<
    string | undefined
  >(undefined);
  const [idComponentSelected, setIdComponentSelected] = useState<
    string | undefined
  >(undefined);
  const [nombreComponentSelected, setNombreComponentSelected] = useState<
    string | undefined
  >(undefined);
  const [componentsForTraining, setComponentsForTraining] = useState<
    IComponente[]
  >([]);
  const [campainsEquipo, setCampainsEquipo] = useState<ICampania[]>([]);
  const [loadingCampainsEquipo, setLoadingCampainsEquipo] =
    useState<boolean>(false);
  const [loadingParametrosReferenciales, setLoadingParametrosReferenciales] =
    useState<boolean>(false);
  ...
  const [tableImportDataProfiles, setTableImportDataProfiles] = useState<
    Array<Array<any>>
  >([]);
  const [tableImportDataCampaigns, setTableImportDataCampaigns] = useState<
    any[][]
  >([]);
  const [headerTableImportDataOp, setHeaderTableImportDataOp] = useState<
    IColumnsTable[]
  >([]);
  const [headerTableImportDataProfiles, setHeaderTableImportDataProfiles] =
    useState<IColumnsTable[]>([]);
  const [headerTableImportDataCampaigns, setHeaderTableImportDataCampaigns] =
    useState<IColumnsTable[]>([]);
  const [headerTableDataOp, setHeaderTableDataOp] = useState<IColumnsTable[]>(
    []
  );

  const [ErrorEstructuraCampaign, setIsErrorEstructuraCampaign] = useState<
    string | undefined
  >(undefined);
  ...
  const [equipoParametros, setEquipoParametros] = useState<
    IParametrosReferencias | undefined
  >(undefined);
  ...
  const updateComponentes = async (equipoId: string) => {
    ...
    await ax
      .get<IComponente[]>("service_render/equipos/componentes_asignados", {
        params: { equipo_id: equipoId },
      })
      .then((response) => {
        setComponentsForTraining(response.data);
        setIdComponentSelected(
          response.data.length > 0 ? response.data[0].id : undefined
        );
        setNombreComponentSelected(
          response.data.length > 0 ? response.data[0].nombre : undefined
        );
      })
      .catch((e: AxiosError) => {
        if (e.response) {
  ...
  const downloadExcel = async () => {
    ...
    const nombreArchivo = `${equipoNombre}_${componentNombre}_${$m().format(
      "YYYYMMDDHHmmss"
    )}.xlsx`;
    const params = {
    ...
    await ax
      .get($j("service_render", "data_pi"), { responseType: "blob", params })
  ...
  const uploadExcelDataOp = async () => {
    ...
    idEquipoSelected !== undefined &&
      formData.append("equipoId", idEquipoSelected);

    setLoading(true);
    modalConfigEquipo.hide();
    await ax
      .post("service_render/data_pi/update", formData, headers)
      .then((response) => {
  ...
  const uploadExcelDataProfiles = async () => {
    ...
    idEquipoSelected !== undefined &&
      formData.append("equipoId", idEquipoSelected);
    idComponentSelected !== undefined &&
      formData.append("componenteId", idComponentSelected);

    setLoading(true);
    modalConfigEquipo.hide();
    await ax
      .post("service_render/data_pi/perfil", formData, headers)
      .then((response) => {
        response.data?.forEach(
          (element: { status: AppearanceTypes; message: string }) => {
  ...
  const uploadExcelDataCampaigns = async () => {
    ...
    modalConfigEquipo.hide();
    await ax
      .post("service_render/campains", formData, headers)
      .then((response) => {
        addToast(response.data?.message, {
          appearance: "success",
          autoDismiss: true,
  ...
  const handleChangeFileOp = async (file: File | null) => {
    ...
    reader.onload = (event) => {
      ...
      const tableData = $x.utils.sheet_to_json<Array<any>>(worksheet, {
        header: 1,
        blankrows: false,
      });
      ...
      const columns = headerRow.map((name: string, index: number) => {
        return {
          ...
          selector: (row: any[]) => {
            if (name === "TIMESTAMP") {
              const date =
                row[index] && !isNaN(row[index])
                  ? DateUtils.excelToDate(
                      $x.SSF.parse_date_code(Math.round(row[index]))
                    )
                  : row[index];
              return date;
            }
  ...
  const handleChangeFileCampaigns = async (file: File | null) => {
    ...
    reader.onload = (event) => {
      ...
      const isValidaEstructura =
        expectedKeys.length === headerRow.length &&
        expectedKeys.every(
          (val, index) => val === headerRow[index].toUpperCase()
        );
      ...
      if (isValidaEstructura) {
        ...
        dataFile.forEach((item, index) => {
          ...
          if (index !== dataFile.length - 1) {
            ...
            if (
              undefinedIndices.length === 1 &&
              undefinedIndices[0] !== columnFechaFin
            ) {
              isValidData = false;
              return;
            }
  ...
  const getDataEquipo = async (selectedTab: string | null) => {
    if (selectedTab === "viewCampaigns") {
      ...
      await ax
        .get<ICampania[]>("service_render/campains", {
          params: {
            equipoId: idEquipoSelected,
            componenteId: idComponentSelected,
          },
        ...
        .then((response) => {
          ...
          const isOpenCamp =
            response.data.length > 0 && response.data[0].fecha_fin === null;
          setIsOpenCampaign(isOpenCamp);
          let dataLastCampaign: ICampania | undefined;
          if (response.data.length > 0) {
            dataLastCampaign = { ...response.data[0] };
            if (isOpenCamp) {
              dataLastCampaign.fecha_inicio = $m(
                response.data[0].fecha_inicio
              ).format("DD-MM-YYYY");
              dataLastCampaign.fecha_fin = $m(
                response.data[0].fecha_inicio
              ).format("DD-MM-YYYY");
            } else {
          ...
          setSelectedCampaingsFromPol(
            response.data
              .filter((item) => item.pol)
              .map((item) => item.numero_camp) as number[]
          );
    ...
    if (selectedTab === "viewParametros") {
      ...
      await ax
        .get<IParametrosReferencias>("service_render/parametros_referencia", {
          params: {
            equipoId: idEquipoSelected,
            componenteId: idComponentSelected,
          },
  ...
  const getDatosOperacionales = async () => {
    ...
    await ax
      ...
      .then((response) => {
        const columnsDataOperacional = response.data.header.map(
  ...
  const saveDataCampaign = async () => {
    setLoadingCampainsEquipo(true);
    await ax
      .post<ICampania[]>("service_render/campains/single", {
        equipoId: idEquipoSelected,
        componenteId: idComponentSelected,
  ...
  const ejecutarProyeccion = async () => {
    setLoading(true);
    await ax
      .post("service_render/proyeccion_pl", {
        equipoId: idEquipoSelected,
        componenteId: idComponentSelected,
      })
      .then((response) => {
  ...
  //GUARDAR PARAMETROS
  const guardarParametrosReferenciales = async (
    data: IParametrosReferencias
  ) => {
    setIsSavingParametros(true);
    await ax
      .post("service_render/parametros_referencia", data)
      .then((response) => {
        modalConfigEquipo.hide();
  ...
  useEffect(() => {
    ...
    setTableDataOp([]);
    if (idComponentSelected == undefined) {
      return;
    }
    getDatosOperacionales();
  }, [idComponentSelected]);
  ...
  const importCampaignsElement: JSX.Element = (
    <>
      ...
      <Col>
        <Tabs
          id="tabsCampaigns"
          defaultActiveKey="viewCampaigns"
          className="border rounded-top"
          onSelect={getDataEquipo}
        >
          <Tab
            eventKey="viewCampaigns"
            title={"Campañas actuales"}
            className="border border-top-0 rounded-bottom"
          >
            {!loadingCampainsEquipo && (
              <Col
                className="py-3 d-flex justify-content-end align-items-end"
              ...
              >
                {!IsOpenCampaign && (
                  <Datepicker
                    ...
                    value={
                      LastCampaign?.fecha_inicio
                        ? LastCampaign.fecha_inicio.toString()
                        : undefined
                    }
                    maxDate={
                      LastCampaign?.fecha_fin
                        ? LastCampaign.fecha_fin.toString()
                        : undefined
                    }
                    onChange={(value) => {
                      setLastCampaign((state) =>
                        $u(state, { fecha_inicio: { $set: value } })
                      );
                    }}
                  />
                )}
            ...
            <Col className="py-4">
              ...
              <Col sm={12} className="mt-3 px-0">
                <Alert variant="danger">
                  <Alert.Heading>
                    <i className={"mx-2 fas fa-exclamation-triangle fa-lg"} />
                    Acción irreversible!
                  </Alert.Heading>
                  <hr />
                  <p className="mb-0">
                    Las campañas del componente {nombreComponentSelected} del
                    equipo {nombreEquipoSelected} serán sustituidas por las
                    campañas cargadas en el archivo.
                  </p>
                </Alert>
              </Col>
              ...
              <Col className="pt-3 d-flex justify-content-end align-items-center ">
                <Button
                  ...
                  onClick={() => {
                    uploadExcelDataCampaigns();
                  }}
                  disabled={
                    tableImportDataCampaigns.length === 0 ||
                    !idComponentSelected ||
                    ErrorEstructuraCampaign !== undefined
                  }
                >
                  <i className={"mx-2 fas fa-file-upload fa-lg"} />
                  <span className="mx-2">
                    Actualizar campañas de {nombreEquipoSelected}
                  </span>
                </Button>
              </Col>
              <Col
                hidden={!ErrorEstructuraCampaign}
    ...
    <>
      <Col className="px-0 py-4">
        ...
        <Col sm={12} className="mt-3">
          <Alert variant="danger">
            <Alert.Heading>
              <i className={"mx-2 fas fa-exclamation-triangle fa-lg"} /> Acción
              irreversible!
            </Alert.Heading>
            <hr />
            <p className="mb-0">
              Los espesores del componente {nombreComponentSelected} para el
              equipo {nombreEquipoSelected} serán sustituidos por los espesores
              cargados en el archivo.
            </p>
          </Alert>
        </Col>
        <Col className="pt-3 d-flex justify-content-end align-items-center ">
          <Button
            className="d-flex justify-content-center align-items-center"
            onClick={() => {
              uploadExcelDataProfiles();
            }}
            disabled={
              !idComponentSelected || tableImportDataProfiles.length === 0
            }
          >
            <i className={"mx-2 fas fa-file-upload fa-lg"} />
  ...
  const importDataOperationalElement: JSX.Element = (
    <>
      <Col className="px-0 py-4">
        <Col>
          <p style={{ fontSize: "14px", fontWeight: 600 }}>
            Actualizar datos operacionales del equipo {nombreEquipoSelected}
          </p>
        </Col>
        <Col className="px-0">
          <Col className="d-flex justify-content-start align-items-center ">
          ...
          <Col sm={12} className="mt-3">
            <Alert variant="danger">
              ...
              <hr />
              <p className="mb-0">
                Los datos operacionales del equipo {nombreEquipoSelected} serán
                sustituidos por los datos de las variables en el archivo.
              </p>
            </Alert>
          </Col>
          <Col className="pt-3 d-flex justify-content-end align-items-center ">
            <Button
              ...
              disabled={tableImportDataOp.length === 0}
            >
              <i className={"mx-2 fas fa-file-upload fa-lg"} />
  ...
  const modalConfigEquipoElement: JSX.Element = (
    <>
      <Modal
        size="xl"
        show={modalConfigEquipo.visible}
        onHide={modalConfigEquipo.hide}
        onShow={() => getDataEquipo("viewCampaigns")}
      >
        <Modal.Header closeButton>
          <b>
            {" "}
            Configuración: {nombreEquipoSelected} / {nombreComponentSelected}{" "}
          </b>
        </Modal.Header>
        <Modal.Body>
          <Tabs
          ...
          >
            <Tab
              ...
              title={
                <>
                  <i className={"mx-2 fas fa-calendar-week"} /> Campañas{" "}
                </>
              }
              className="border border-top-0 rounded-bottom"
            >
            ...
            <Tab
              eventKey="viewPerfiles"
              title={
                <>
                  <i className={"mx-2 fas fa-ruler"} /> Mediciones{" "}
                </>
              }
              className="border border-top-0 rounded-bottom"
            >
            ...
            <Tab
              eventKey="viewImportar"
              title={
                <>
                  <i className={"mx-2 fas fa-file-upload"} /> Importar{" "}
                </>
              }
              className="border border-top-0 rounded-bottom"
            >
            ...
            <Tab
              eventKey="viewParametros"
              title={
                <>
                  <i className={"mx-2 fas fa-cog"} /> Parametros{" "}
                </>
              }
              className="border border-top-0 rounded-bottom"
            >
      ...
      <BaseContentView title="titles:data_op">
        ...
        <Col md={3}>
          <ApiSelect<IComponente>
            ...
            errors={
              componentsForTraining.length === 0 && !loadingData
        ...
        <Col
          xl={6}
          className="px-0 d-sm-flex justify-content-end align-items-start"
        >
          ...
          <Col sm={3} className="pt-2">
            ...
            <Button
              ...
              disabled={
                loadingData ||
                componentsForTraining.length === 0 ||
                tableDataOp.length === 0
              }
            >
