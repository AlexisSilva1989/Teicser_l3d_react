...
export const useApi = (api: AxiosInstance = ax) => {
	...
	const getCallback = useCallback(<T extends unknown>(url: string, config?: AxiosRequestConfig) => {
		return {
			success: (dataCallback: (e: T) => void) => {
				return {
					fail: async (error: string|null, action?: () => void) => {
						await api.get(url, config).then(e => {
	...
	const postCallback = useCallback(<T extends unknown>(url: string, data?: unknown, config?: AxiosRequestConfig) => {
		return {
			success: (dataCallback?: (e: T) => void) => {
				return {
	...
	const patchCallback = useCallback(<T extends unknown>(url: string, data?: unknown, config?: AxiosRequestConfig) => {
		return {
			success: (dataCallback?: (e: T) => void) => {
				return {
					fail: async (error: string, params?: any) => {
						await api.patch(url, data, config).then(e => {
							if (dataCallback) { 
								dataCallback(e.data); 
