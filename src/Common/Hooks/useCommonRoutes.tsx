...
export const useCommonRoutes = () => {
	...
	const gotoAddCallback = useCallback((state?: any) => pushTo('routes:meta.add', state), [pushTo]);
	//const gotoModifyCallback = useCallback((state?: any) => pushTo('routes:meta.modify', state), [pushTo]);
	const gotoDetailsCallback = useCallback((state?: any) => pushTo('routes:meta.details', state), [pushTo]);

	const gotoModifyCallback = useCallback((state?: any ,  innerPath?:string ) => {
	...
	}, [ goBackCallback, gotoAddCallback, gotoModifyCallback, gotoDetailsCallback]);
