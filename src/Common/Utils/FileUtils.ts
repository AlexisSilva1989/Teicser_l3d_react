...
export class FileUtils {
	...
	static sheetToArray<T = unknown>(workbook: IWorkbook, index = 0, range?: string, columns?: string[]) {
		const sheet = workbook.Sheets[workbook.SheetNames[index]];
		const arr = $x.utils.sheet_to_json(sheet, { header: columns ?? undefined, range });
		return arr as T[];
	}

	static sheetToJson(workbook: IWorkbook, index = 0, range?: string, columns?: string[]) {
		const array = FileUtils.sheetToArray(workbook, index, range, columns);
		return JSON.stringify(array);
	}

	static async readFileToArray<T = unknown>(file: File, sheet = 0, range?: string, columns?: string[]) {
