...
export const NavGroup = (props: Props) => {
	...
	const navItems = props.group.submenus?.map((item, i) => {
		return <EnhancedNavCollapse key={i} collapse={item} type='main' badgeData={props.badges} />;
	});

	const navMods = props.group.modules?.map((item, i) => {
		return <EnhancedNavItem key={i} item={item} badgeCount={badgeCount(item.badgeId)} />;
	});
