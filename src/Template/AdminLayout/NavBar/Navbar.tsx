...
const NavBar = (props: Props) => {
	...
	let headerClass = ['navbar', 'pcoded-header', 'navbar-expand-lg', 'header-dark'];

	document.body.classList.remove('background-blue', 'background-red', 'background-purple', 'background-info', 'background-green', 'background-dark');

	document.body.classList.remove('background-grd-blue', 'background-grd-red', 'background-grd-purple', 'background-grd-info', 'background-grd-green', 'background-grd-dark');

	document.body.classList.remove('background-img-1', 'background-img-2', 'background-img-3', 'background-img-4', 'background-img-5', 'background-img-6');
	...
	let navBar = (
		<Fragment>
			<div className='m-header'>
				<a className={toggleClass.join(' ')} id='mobile-collapse1' href={Links.BLANK} onClick={props.toggleNav}>
					<span />
				</a>
				<NavLink to='/' className='b-brand'>
					...
					<span className='ml-3 login-brand' style={{ fontFamily: 'helvetica' ,  textTransform: 'uppercase' }}>
	...
	return (
		<Fragment>
			<header className={headerClass.join(' ')} style={{ background: 'var(--dark)' }}>
				{navBar}
			</header>
		</Fragment>
	);
...
const mapProps = (state: IAppState): Pick<Props, 'headerColor' | 'fixedHeader' | 'collapseMenu' | 'layout' | 'sublayout'> => {
	return {
