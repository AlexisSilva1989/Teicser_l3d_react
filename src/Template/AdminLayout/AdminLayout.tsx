...
export const AdminLayout = () => {
	const { fullscreen, collapseMenu, layout, sublayout, contentOnly, showConfiguration, toggleNav, setFullscreen } = useTemplate();
	...
	useEffect(() => {
		if (window.outerWidth > 992 && window.outerWidth <= 1024 && layout !== 'horizontal') {
			toggleNav();
		}
	});


	function mobileOutClickHandler() { if (window.outerWidth < 992 && collapseMenu) { toggleNav(); } }
	...
	const menusMemo = useMemo(() => {
		return routes.map((group) => {
			return group.routes?.map((route, index) => {
				...
				const path = localize(route.path, route.element ? { element: localize(route.element) } : undefined);
                 
				const pushedPath = $j('/', prefix === '/' ? '' : prefix, path);
					
				setPaths(state => [...state, pushedPath]);
				return <Route key={index} path={pushedPath} exact={route.exact == null ? true : route.exact}>
					{route.component}
				</Route>;
			});
		});
