...
export const SelectIdNombre = (e: IdNombre): ValueDisplay => {
  return { display: e.nombre, value: e.id.toString() };
};
export const SelectIdNombreOption = (e: IdNombre): DropdownOption => {
  return { name: e.nombre, value: e.id.toString() };
};

export const MapIdNombre: (
  data: IdNombre[],
  addNone?: boolean,
  caps?: (e: string) => string
) => { name: string; value: string }[] = (data, addNone, caps) => {
  let mapped = data.map((x) => {
    return { name: x.nombre, value: x.id.toString() };
  });
  if (addNone) {
    mapped = [
      { name: caps ? caps("labels:meta.none") : "N/A", value: "-1" },
      ...mapped,
    ];
  }
  return mapped;
...
const IntlCLP = new Intl.NumberFormat("es-CL", {
  style: "currency",
  currency: "CLP",
});
...
export class Format {
  ...
  static parse_number_format = (
    number: number | string,
    decimals: number = 2,
    separator: string = "point"
  ) => {
    ...
    return new Intl.NumberFormat(formats["point"], {
